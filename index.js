...
app.get("/api/hello", async (req, res) => {
  ...
  try {
    let clientIp = await axios.get(`https://ipinfo.io/json?token=${token}`); //http://ip-api.com/json/
    let clientIpData = await clientIp.data;

    let city = clientIpData.city;
    let ip = clientIpData.ip;
